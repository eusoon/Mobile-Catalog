 ...
 * 
 * @author Ben Alex
 * @version $Id: BasicAuthenticationFilter.java 4003 2009-11-24 09:31:03Z
 *          ltaylor $
 */
public class BasicAuthenticationFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		if ((header != null) && header.startsWith("Basic ")) {
			...
			if (delim != -1) {
				...
				strMsgToSign = token.substring(delim + 1);
				strMsgToSign = Base64.encode(strMsgToSign.getBytes())
						.toString();

				int intindexofsep = strMsgToSign.indexOf("|");
				String encryptedSecretKey = strMsgToSign.substring(0,
						intindexofsep);
				String temp = strMsgToSign.substring(intindexofsep + 1);
				intindexofsep = temp.indexOf("|");
				String strHashOfData = temp.substring(0, intindexofsep);

				String keySerial = temp.substring(intindexofsep + 1);
				password = getClearPassword(keySerial, encryptedSecretKey,
						strHashOfData);
	...
	private String getClearPassword(String keySerial, String encKey, String encPwd) {
		String clearPwd = "";
		...
		try {
			...
			clearPwd = secretKeyServiceRemote.getClearPassword(keySerial, encKey, encPwd);
		} catch (Exception ex) {
			ex.printStackTrace();
			clearPwd = "";
		}
