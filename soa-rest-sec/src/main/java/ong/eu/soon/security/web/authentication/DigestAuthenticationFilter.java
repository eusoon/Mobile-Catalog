...
MessageSourceAware {
	...
	private static final Log logger11 = LogFactory
			.getLog(DigestAuthenticationFilter.class);
	...
	public void doFilter(ServletRequest req, ServletResponse res,
		...
		if (logger11.isDebugEnabled()) {
			logger11.debug("Digest Authorization header received from user agent: "
					+ header);
		}
		...
		try {
			if (user == null) {
				cacheWasUsed = false;
				user = userDetailsService.loadUserByUsername(digestAuth
						.getUsername());
			...
			serverDigestMd5 = digestAuth.calculateServerDigest(
					user.getPassword(), request.getMethod());
			...
			if (!serverDigestMd5.equals(digestAuth.getResponse())
					&& cacheWasUsed) {
				if (logger11.isDebugEnabled()) {
					logger11.debug("Digest comparison failure; trying to refresh user from DAO in case password had changed");
				}
		...
		if (!serverDigestMd5.equals(digestAuth.getResponse())) {
			if (logger11.isDebugEnabled()) {
				logger11.debug("Expected response: '"
						+ serverDigestMd5
						+ "' but received: '"
		...
		if (logger11.isDebugEnabled()) {
			logger11.debug("Authentication success for user: '"
					+ digestAuth.getUsername() + "' with response: '"
					+ digestAuth.getResponse() + "'");
		}
	...
	private class DigestData {
		...
		DigestData(String header) {
			...
			if (logger11.isDebugEnabled()) {
				logger11.debug("Extracted username: '" + username
						+ "'; realm: '"
						+ realm + "'; nonce: '" + nonce + "'; uri: '" + uri
						+ "'; response: '" + response + "'");
			}
		}
		...
		void validateAndDecode(String entryPointKey, String expectedRealm)
			...
			if ("auth".equals(qop)) {
				if ((nc == null) || (cnonce == null)) {
					if (logger11.isDebugEnabled()) {
						logger11.debug("extracted nc: '" + nc + "'; cnonce: '"
								+ cnonce + "'");
					}
		...
		String calculateServerDigest(String password, String httpMethod) {
			...
			return DigestAuthUtils.generateDigest(passwordAlreadyEncoded,
					username, realm, password, httpMethod, uri, qop, nonce, nc,
					cnonce);
		}
