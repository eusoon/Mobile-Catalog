...
import ong.eu.soon.utils.binding.PropertyException;
import ong.eu.soon.utils.conversion.CoercionHelper;
import ong.eu.soon.utils.conversion.Converter;
...
public class PropertyHelper {
	...
	/**
	 * Copies all possible properties from source to dest, ignoring the given
	 * properties list. Exceptions are ignored
	 */
	public static void copyProperties(final Object source, final Object dest,
		...
		for (final PropertyDescriptor sourceDescriptor : propertyDescriptors) {
			try {
				...
				final PropertyDescriptor destProperty = PropertyUtils
						.getPropertyDescriptor(dest, name);
				if (destProperty.getWriteMethod() == null) {
					// Ignore read-only properties
					continue;
				}
				final Object value = CoercionHelper.coerce(
						destProperty.getPropertyType(), get(source, name));
				set(dest, name, value);
	 ...
	 */
	public static <T> T getAsObject(final Class<T> toType, final String value,
	 ...
	 */
	public static <T> String getAsString(final T value,
			final Converter<T> converter) {
		if (value == null) {
			return null;
		}
	 ...
	 */
	public static void set(final Object object, final String property,
			final Object value) {
		if (object == null) {
			return;
		}
